...
public class MediaListActivity extends ListActivity {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		{
			XBMCControl.getInstance().getMediaControls().playFile(item.url);
		}
	}
	...
	private void fillUp(String url) {
		...
		if (url.length() == 0)
		{
			for (Item item : XBMCControl.getInstance().getShares(mediaType))
			{
		...
		{
			for (String s : XBMCControl.getInstance().getDirectory(url))
			{
